...
- **Bloat** — lines that restate the model's default, duplicate a reference, or no longer bear on the project (core.md's Per-line quality tests)
...
One metric per pass. A file needing several gets sequential passes, run autonomously in order — bloat first, since trimming first means less material to reformat or reorder. If the file is nearing the ~200-line rung-1 ceiling or an area's section is swelling, that is a graduation signal, not an improve target: hand it to update-instructions (its Graduation step executes the move under plan approval) instead of compressing around the problem.
...
Check the constitution's sections against `instruction-files.md`'s AGENTS.md section playbook: fixed order, each section optional, content-rule violations (folder internals leaking into the constitution instead of a reference; a fact restated instead of linked). Check each `references/` file is scoped to one area per that module's References section. Move misplaced content to where the playbook says it belongs; do not invent new sections the playbook doesn't name.
...
Report the delta: metric chosen, before/after sizes, the fact-survival table, and any graduation signal spotted in Step 1 for the user to act on separately. Then run the mandatory cleanup: archive the snapshot and this pass's log to `~/.claude/skill-archive/`, and `rm -rf` the workspace — leave nothing under `~/.claude/skill-workspaces/`.
