...
- **[hooks/](hooks/)** — enforcement across five surfaces. A **guard** hard-denies create/delete/large edits of skills and agents (routing them through the lifecycle) while letting a one-line edit through with a nudge, and hard-gates three prevention-critical surfaces with no small-edit escape: hook-config changes, direct edits to project instruction files (`CLAUDE.md`/`AGENTS.md`), and edits to upstream/vendored skills (any folder carrying an `.upstream` marker — blocked even inside the lifecycle, so vendored content is never locally forked by accident). Bash mutations are matched by operand-adjacency, so a filename merely named in a commit message never trips the guard. A **md-format** hook nudges (never blocks) on any `.md` write and warns when a constitution outgrows the ~200-line ladder threshold.
