...
- **[hooks/](hooks/)** — enforcement across four surfaces. A **guard** hard-denies create/delete/large edits of skills and agents (routing them through the lifecycle) while letting a one-line edit through with a nudge, and hard-gates two prevention-critical surfaces with no small-edit escape: hook-config changes and direct edits to project instruction files (`CLAUDE.md`/`AGENTS.md`), forcing those through the instruction-file lifecycle. A **md-format** hook nudges (never blocks) on any `.md` write and warns when a constitution outgrows the ~200-line ladder threshold.
